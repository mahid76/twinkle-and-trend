...
*Message:* ${formData.message}`;
  ...
  return (
    // The container uses brand color with low opacity as a subtle background
    <div className="w-full min-h-screen py-8 sm:py-12 md:py-16 px-4 sm:px-6 lg:px-12 bg-[#E771A3]/20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8 sm:mb-12 md:mb-16">
          ...
          <p className="mt-2 sm:mt-3 md:mt-4 text-sm sm:text-base md:text-lg lg:text-xl text-gray-600 max-w-2xl mx-auto px-4">
            Have a question about a product or just want to say hello? We'd love to hear from you.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12 xl:gap-24 items-start">
          
          {/* Column 1: Contact Information - Side by side on mobile/tablet, stacked on desktop */}
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4 lg:gap-6">
            {/* Contact Details Card */}
            <div className="bg-white p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl shadow-lg border border-gray-100">
              <h2 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 mb-2 sm:mb-3">
                Contact Details
              </h2>
              <div className="space-y-2 sm:space-y-3">
                {[
                  { icon: '📍', title: 'Location', value: '123 Collectors Lane, New York' },
                  { icon: '📧', title: 'Email', value: 'hello@collectorhq.com' },
                  { icon: '📞', title: 'Phone', value: '+880 1614-326888' },
                ].map((item) => (
                  <div key={item.title} className="flex gap-1.5 sm:gap-2 items-start">
                    <div className="text-lg sm:text-xl text-[#E771A3]">{item.icon}</div>
                    <div>
                      <h4 className="text-[10px] sm:text-xs font-semibold text-gray-900">{item.title}</h4>
                      <p className="text-[8px] sm:text-[10px] text-gray-600 break-words">{item.value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl shadow-lg border border-gray-100">
              ...
              <p className="text-[8px] sm:text-[10px] text-gray-600 mb-2 sm:mb-3">
                Follow us!
              </p>
              <div className="flex flex-col gap-1.5 sm:gap-2">
                {['FB', 'IG', 'TW'].map(social => (
                  <button 
                    key={social} 
                    className="text-left text-[10px] sm:text-xs text-[#E771A3] hover:opacity-70 transition-opacity font-medium"
                  >
                    {social === 'FB' ? 'Facebook' : social === 'IG' ? 'Instagram' : 'Twitter'}
                  </button>
                ))}
              </div>
            </div>

            {/* Quick WhatsApp Contact - Full width on all screens */}
            <div className="col-span-2 md:col-span-2 lg:col-span-1 mt-1 sm:mt-2">
              <div className="bg-white p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl shadow-lg border border-gray-100">
                <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 mb-2">
                  WhatsApp
                </h3>
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 sm:gap-2 bg-[#25D366] text-white px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg hover:bg-[#128C7E] transition-colors text-[10px] sm:text-xs"
                >
          ...
          {/* Column 2: The Contact Form - Responsive */}
          <form onSubmit={handleSubmit} className="bg-white p-5 sm:p-6 md:p-8 lg:p-10 rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-xl border border-gray-100">
            ...
            <div className="space-y-3 sm:space-y-4 lg:space-y-6">
              ...
              <div>
                ...
                <input 
                  ...
                  required
                  placeholder="Jane Doe"
                  className="w-full bg-[#E771A3]/20 text-gray-800 placeholder:text-gray-500 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 text-sm border-none focus:ring-2 focus:ring-[#E771A3]/50 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  required
                  placeholder="How can we help you today?"
                  className="w-full bg-[#E771A3]/20 text-gray-800 placeholder:text-gray-500 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 text-sm border-none focus:ring-2 focus:ring-[#E771A3]/50 outline-none resize-none"
                ></textarea>
              </div>
              ...
              <button 
                type="submit" 
                disabled={isSubmitting}
                className={`w-full flex items-center justify-center gap-2 text-white text-sm sm:text-base font-bold py-2.5 sm:py-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 outline-none ${
                  isSubmitting 
                    ? 'bg-gray-400 cursor-not-allowed' 
                    : 'bg-[#25D366] hover:bg-[#128C7E] hover:scale-[1.02] active:scale-[0.98]'
                }`}
