...
const Products = () => {
  ...
  const getPageNumbers = () => {
    ...
    if (totalPages <= 5) {
      ...
      [1, "...", totalPages - 3, totalPages - 2, totalPages - 1, totalPages].forEach(
        (p) => pages.push(p)
      );
    } else {
      [1, "...", currentPage - 1, currentPage, currentPage + 1, "...", totalPages].forEach(
        (p) => pages.push(p)
      );
    }
    return pages;
        ...
        <div className="flex flex-wrap gap-2 mb-4 md:mb-8">
          {categories.map((cat) => {
            ...
            return (
              <button
                key={cat}
                onClick={() => { setSelectedCategory(cat === "All" ? "" : label); handleFilterChange(); }}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  (cat === "All" && selectedCategory === "") || selectedCategory === label
                ...
                return (
                  ...
                  >
                    <div className="relative overflow-hidden">
                      ...
                      {product.stock === 0 && (
                        <div className="absolute top-2 left-2 bg-red-500 text-white px-2 md:px-3 py-1 text-[10px] md:text-sm rounded-full font-medium">Out of Stock</div>
                      )}
                      {product.stock < 10 && product.stock > 0 && (
                        <div className="absolute top-10 left-2 bg-[#E771A3] text-white px-2 md:px-3 py-1 text-[10px] md:text-sm rounded-full font-medium">Only {product.stock} left!</div>
                      )}
                      {hasOffer && (
                        <div className="absolute bottom-2 left-2 bg-[#E771A3] text-white px-2 md:px-3 py-1 text-[10px] md:text-sm rounded-full font-bold">{discount}% OFF</div>
                      )}
