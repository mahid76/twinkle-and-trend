        ...
        <Container>
            <div className="py-8">
                <div className="text-center mb-12">
                    <h1 className="text-4xl font-primary font-bold text-gray-800 mb-4">Special Offers</h1>
                    <p className="text-gray-600 text-lg">Get amazing deals on selected products</p>
                        ...
                        return (
                            <Link key={product.id} to={`/products/${product.id}`}
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute top-2 left-2 bg-[#E771A3] text-white px-2 py-0.5 text-[10px] md:text-xs rounded-full font-bold">{discount}% OFF</div>
                                    {product.isBestSeller && <div className="absolute bottom-2 left-2 bg-[#F6D6DF] text-[#E771A3] px-2 py-1 rounded-full text-[10px] font-bold">⭐ Best Seller</div>}
                                </div>
                                <div className="p-3 md:p-4">
                                    <h3 className="text-sm md:text-base font-bold text-gray-800 mb-1 line-clamp-2 hover:text-[#E771A3] transition-colors">{product.name}</h3>
                                    <p className="text-gray-500 text-xs mb-2 line-clamp-2 hidden md:block">{product.description}</p>
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="text-lg font-bold text-[#E771A3]">৳{product.offerPrice}</span>
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mb-8">
                        <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1}
                            className={`px-4 py-2 rounded-lg font-medium transition-colors ${currentPage === 1 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#E771A3] text-white hover:bg-[#d15f93]"}`}>
                            Previous
                        </button>
                        {Array.from({ length: totalPages }, (_, index) => (
                            <button key={index + 1} onClick={() => paginate(index + 1)}
                                className={`px-4 py-2 rounded-lg font-medium transition-colors ${currentPage === index + 1 ? "bg-[#E771A3] text-white" : "bg-white text-gray-700 hover:bg-gray-100"}`}>
                                {index + 1}
                            </button>
                        ))}
                        <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === totalPages}
                            className={`px-4 py-2 rounded-lg font-medium transition-colors ${currentPage === totalPages ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#E771A3] text-white hover:bg-[#d15f93]"}`}>
                            Next
                        </button>
                    </div>
                )}
