	...
	<Container>
		<div className="py-4 md:py-6 animate-pulse">
			<div className="flex gap-2 mb-6">
				{[10, 2, 16, 2, 32].map((w, i) => (
					<div key={i} className={`h-4 bg-gray-200 rounded w-${w}`} />
				))}
			</div>
			<div className="block md:flex gap-10">
					...
					<div className="hidden md:flex gap-3">
						...
						<div className="flex flex-col gap-2">
							{[1, 2, 3].map((i) => (
								<div
									key={i}
									className="w-[100px] aspect-[4/5] bg-gray-200 rounded-lg"
								/>
							))}
						</div>
					</div>
				...
				<div className="flex-1 mt-5 md:mt-0 space-y-4">
					...
					<div className="flex gap-2">
						{[1, 2, 3].map((i) => (
							<div key={i} className="w-12 h-8 bg-gray-200 rounded" />
						))}
					</div>
...
const ZoomPanel = ({
	...
	return (
		<div
			className="hidden lg:block fixed z-40 rounded-xl overflow-hidden shadow-2xl border border-gray-200"
			style={style}
		>
			<div
				className="w-full h-full"
...
const HoverZoomImage = ({ image, children }) => {
	...
	const onMove = (e) => {
		...
		const x = e.clientX - r.left,
			y = e.clientY - r.top;
		setLensPos({
			x: Math.min(Math.max(x - L / 2, 0), r.width - L),
			y: Math.min(Math.max(y - L / 2, 0), r.height - L),
		});
			...
			>
				...
				{hovered && (
					<div
						...
						style={{ left: lensPos.x, top: lensPos.y, width: L, height: L }}
					/>
				)}
			</div>
			{hovered && (
				<ZoomPanel src={image} zoomPos={zoomPos} containerRef={ref} />
			)}
		</>
	);
};
...
const VariantSwatch = ({ variant, isSelected, onClick }) => {
	...
	const onEnter = () => {
		...
		setPStyle({
			...
			top: Math.max(
				10,
				Math.min(r.top + r.height / 2 - P / 2, window.innerHeight - P - 10),
			),
			width: P,
			height: P,
		});
	...
	return (
		<>
			...
			{hovered && variant.images?.[0] && (
				<div
					className="hidden lg:block fixed z-50 rounded-xl overflow-hidden shadow-2xl border-2 border-[#E771A3] pointer-events-none"
					style={pStyle}
				>
					<img
...
const ProductDetail = () => {
	...
	useEffect(() => {
		...
		if (found.variants?.length > 0) {
			const matched = colorParam
				? found.variants.find((v) => v.color === colorParam)
				: null;
		...
		return (
			<Container>
				<div className="text-center py-20">
					...
					<Link to="/products" className="text-[#E771A3] hover:underline">
						← Back to Shop
					</Link>
				</div>
			</Container>
	...
	const buildOrderMessage = () =>
		`
*New Order — Twinkle and Trend BD*
			...
			<div className="block md:flex md:items-start mb-8 md:mb-12 md:gap-5 lg:gap-10">
				...
				<div className="flex-shrink-0">
					...
					<div className="hidden md:flex items-start gap-3">
						<HoverZoomImage image={currentHdImage}>
							<div
								className="relative overflow-hidden rounded-lg w-full md:w-[280px] lg:w-[380px] xl:w-[450px] aspect-[4/5]"
							...
							>
								...
								<div
									className="hidden md:flex absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/40 text-white text-xs px-3 py-1 rounded-full items-center gap-1.5 pointer-events-none"
									aria-hidden="true"
								>
									<svg
										className="w-3 h-3"
						...
						<div className="flex flex-col gap-2">
							{activeImages.map((img, i) => (
								...
								>
									<div
										className="w-[75px] lg:w-[100px] aspect-[4/5]"
										onContextMenu={(e) => e.preventDefault()}
									>
										<img
											src={cldWebp(img, 200)}
					...
					<div className="md:hidden">
						<div
							...
							style={{ maxWidth: "85%", aspectRatio: "4/5" }}
							onContextMenu={(e) => e.preventDefault()}
						>
							<img
								src={cldWebp(currentImage, 600)}
								alt={displayName}
								loading="eager"
								fetchPriority="high"
								draggable={false}
				...
				<div className="flex-1 mt-5 md:mt-0">
					...
					<div className="flex items-center gap-3 mb-4">
						<span className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#E771A3]">
							৳{displayPrice}
						</span>
						{originalPrice && (
							<>
					...
					<button
						...
						aria-label={
							wishlisted ? "Wishlist থেকে সরান" : "Wishlist এ যোগ করুন"
						}
					...
					<p className="text-gray-600 leading-relaxed whitespace-pre-line text-sm md:text-base mb-5">
							...
							<div className="flex flex-wrap gap-3">
								{product.variants.map((v, i) => (
									<VariantSwatch
										key={i}
										variant={v}
										isSelected={selectedVariant?.color === v.color}
						...
						<div className="mb-5">
							...
							<div className="flex flex-wrap gap-2">
								{product.sizes.map((s, i) => (
									<button
										key={i}
										onClick={() => s.stock > 0 && handleSizeChange(s)}
										disabled={s.stock === 0}
					...
					<div className="flex items-center gap-4 mb-5">
						...
						<span
							className={`text-sm font-medium ${activeStock > 0 ? "text-green-600" : "text-red-500"}`}
						>
              ...
              Buy Now on WhatsApp
					...
					<div className="mt-6 pt-5 border-t border-gray-100 space-y-2 text-sm text-gray-600">
						...
						<div className="flex gap-2">
							<span className="font-semibold text-gray-800 w-20">Brand:</span>
							<span>China</span>
						</div>
					</div>
						...
						<div className="mt-5 pt-4 border-t border-gray-100">
							...
							<ul className="space-y-1.5">
								{product.features.map((f, i) => (
									...
									>
										<svg
											className="w-4 h-4 text-[#E771A3] mt-0.5 flex-shrink-0"
											fill="none"
											stroke="currentColor"
											strokeWidth={2.5}
											viewBox="0 0 24 24"
