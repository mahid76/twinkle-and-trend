...
// ─── Skeleton Loading ─────────────────────────────────────────────────────────
// ✅ Skeleton দেখাবে শুধু প্রথমবার lazy chunk load হওয়ার সময়
// তারপর navigate করলে আর দেখাবে না — data আগে থেকেই আছে
    ...
    <div className="py-4 md:py-6 animate-pulse">
      ...
      <div className="block md:flex gap-10">
        ...
        <div className="flex-shrink-0">
          ...
          {/* Mobile */}
          <div className="md:hidden mx-auto" style={{ maxWidth: "75%" }}>
            <div className="aspect-[4/5] bg-gray-200 rounded-lg" />
            <div className="flex justify-center gap-2 mt-3">
              {[1,2,3].map(i => <div key={i} className="w-[60px] aspect-[4/5] bg-gray-200 rounded-lg" />)}
            </div>
          </div>
        </div>
        {/* Details column */}
        <div className="flex-1 mt-5 md:mt-0 space-y-4">
          ...
          <div className="flex gap-2">
            {[1,2,3].map(i => <div key={i} className="w-9 h-9 bg-gray-200 rounded-full" />)}
          </div>
          {/* Size */}
          <div className="flex gap-2">
            {[1,2,3].map(i => <div key={i} className="w-12 h-8 bg-gray-200 rounded" />)}
          </div>
          {/* Buttons */}
...
const ImageZoomModal = ({ image, onClose }) => {
  ...
  const handleTouchMove = (e) => {
    ...
    setPos((prev) => ({ x: Math.min(100, Math.max(0, prev.x - dx)), y: Math.min(100, Math.max(0, prev.y - dy)) }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black">
      <div className="flex items-center justify-between px-4 py-3 bg-black/80 flex-shrink-0">
        ...
        <button onClick={onClose} className="bg-white/20 hover:bg-white/30 text-white rounded-full w-9 h-9 flex items-center justify-center transition-colors">✕</button>
      </div>
...
const ZoomPanel = ({ src, zoomPos, containerRef, panelSize = 380, zoom = 3 }) => {
  ...
  useEffect(() => {
    ...
    if (left + panelSize > window.innerWidth - 10) left = rect.left - panelSize - 20;
    let top = rect.top;
    if (top + panelSize > window.innerHeight - 10) top = window.innerHeight - panelSize - 10;
    setStyle({ left, top, width: panelSize, height: panelSize });
  }, [containerRef, panelSize]);

  return (
    <div className="hidden md:block fixed z-40 rounded-xl overflow-hidden shadow-2xl border border-gray-200 bg-gray-50" style={style}>
      <div className="w-full h-full" style={{ backgroundImage: `url(${src})`, backgroundRepeat: "no-repeat", backgroundSize: `${zoom * 100}%`, backgroundPosition: `${zoomPos.x}% ${zoomPos.y}%` }} />
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none" aria-hidden="true">
...
const HoverZoomImage = ({ image, children }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    setLensPos({ x: Math.min(Math.max(x - LENS / 2, 0), rect.width - LENS), y: Math.min(Math.max(y - LENS / 2, 0), rect.height - LENS) });
    setZoomPos({ x: (x / rect.width) * 100, y: (y / rect.height) * 100 });
  };

  return (
    <>
      <div ref={containerRef} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onMouseMove={handleMouseMove} className="relative cursor-crosshair">
        {children}
        {hovered && <div className="absolute border-2 border-[#E771A3] bg-white/20 pointer-events-none z-20" style={{ left: lensPos.x, top: lensPos.y, width: LENS, height: LENS }} />}
      </div>
      {hovered && <ZoomPanel src={image} zoomPos={zoomPos} containerRef={containerRef} />}
    </>
  );
};
...
const VariantSwatch = ({ variant, isSelected, onClick }) => {
  ...
  const handleMouseEnter = () => {
    ...
    if (left + PANEL > window.innerWidth - 10) left = rect.left - PANEL - 12;
    let top = Math.max(10, Math.min(rect.top + rect.height / 2 - PANEL / 2, window.innerHeight - PANEL - 10));
    setPanelStyle({ left, top, width: PANEL, height: PANEL });
    setHovered(true);
  };

  return (
    <>
      <div ref={ref} onMouseEnter={handleMouseEnter} onMouseLeave={() => setHovered(false)} onClick={() => variant.stock > 0 && onClick(variant)} className="relative">
        <div title={variant.color}
          className={`relative w-9 h-9 rounded-full border-2 transition-all ${isSelected ? "border-[#E771A3] scale-110 shadow-md" : "border-gray-300 hover:border-gray-400 hover:scale-105"} ${variant.stock === 0 ? "opacity-40 cursor-not-allowed" : "cursor-pointer"}`}
      ...
      {hovered && variant.images?.[0] && (
        <div className="hidden md:block fixed z-50 rounded-xl overflow-hidden shadow-2xl border-2 border-[#E771A3] pointer-events-none" style={panelStyle}>
...
const ProductDetail = () => {
  ...
  // ─── Derived values ───────────────────────────────────────────────────────
  // ✅ FIX: name এখন variant থেকে আসে, না থাকলে product name
  ...
  // Size এর extra price যোগ হবে variant price এর উপর
  const sizeExtraPrice = selectedSize?.extraPrice || 0;
  ...
  const baseOriginalPrice = selectedVariant?.offerPrice && selectedVariant.offerPrice < selectedVariant.price
    ? selectedVariant.price
    : product?.offerPrice && product.offerPrice < product.price ? product.price : null;

  const displayPrice = (basePrice || 0) + sizeExtraPrice;
  const originalPrice = baseOriginalPrice ? baseOriginalPrice + sizeExtraPrice : null;
  ...
  const handleQuantityChange = (delta) => {
    setQuantity((prev) => {
      ...
      return next >= 1 && next <= activeStock ? next : prev;
    });
  };
...
*SKU:* T&T-${product?.id}${selectedVariant ? `-${selectedVariant.color?.slice(0,3).toUpperCase()}` : ""}
    ...
    <Container>
      ...
      <div className="block md:flex md:items-start mb-8 md:mb-12 md:gap-10">
          ...
          <div className="hidden md:flex items-start gap-3">
            <HoverZoomImage image={currentHdImage}>
              <div className="relative overflow-hidden rounded-lg" style={{ width: 450, aspectRatio: "4/5" }} onContextMenu={(e) => e.preventDefault()}>
                <img
                  src={cldWebp(currentImage, 900)}
                  srcSet={`${cldWebp(currentImage, 500)} 500w, ${cldWebp(currentImage, 900)} 900w`}
                  sizes="450px"
                  alt={displayName}
                  loading="eager"
                  fetchpriority="high"
            ...
            <div className="flex flex-col gap-2">
              {activeImages.map((img, i) => (
                <button key={i} onClick={() => setActiveImage(i)}
                  ...
                  <div className="relative w-[100px] aspect-[4/5]" onContextMenu={(e) => e.preventDefault()}>
          ...
          <div className="md:hidden">
            <div className="relative overflow-hidden rounded-lg mx-auto" style={{ maxWidth: "75%", aspectRatio: "4/5" }} onContextMenu={(e) => e.preventDefault()}>
              ...
              <button onClick={() => setZoomOpen(true)}
                className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-gray-700 rounded-full px-3 py-1.5 text-xs font-semibold shadow-lg flex items-center gap-1.5 z-10"
                aria-label="HD Zoom">
        ...
        <div className="flex-1 mt-5 md:mt-0">
          ...
          {/* Name — ✅ variant এর name দেখাবে */}
          <h1 className="text-2xl md:text-3xl font-primary font-bold text-gray-800 mb-2 leading-tight">
            {displayName}
          </h1>
          ...
          <div className="flex items-center gap-2 mb-3">
            <div className="flex" aria-label={`Rating: ${product.rating} out of 5`}>
              {[...Array(5)].map((_, i) => (
                <span key={i} className={i < Math.floor(product.rating) ? "text-yellow-500" : "text-gray-300"} aria-hidden="true">★</span>
              ))}
            </div>
          ...
          <button onClick={handleWishlist}
            ...
            aria-label={wishlisted ? "Wishlist থেকে সরান" : "Wishlist এ যোগ করুন"}>
          ...
          {/* Color Variants — ✅ নাম + রং দুটোই বদলায় */}
          {product.variants?.length > 0 && (
            <div className="mb-4">
              <p className="text-gray-700 font-medium mb-2 text-sm">
                Color: <span className="text-[#E771A3] font-semibold">{selectedVariant?.color}</span>
              </p>
          ...
          {/* ✅ Size selector — price ও বদলায় */}
          {product.sizes?.length > 0 && (
            <div className="mb-4">
              <p className="text-gray-700 font-medium mb-2 text-sm">
                Size: <span className="text-[#E771A3] font-semibold">{selectedSize?.label}</span>
                {selectedSize?.extraPrice > 0 && (
                  <span className="text-gray-400 text-xs ml-2">(+৳{selectedSize.extraPrice})</span>
                )}
              </p>
                  ...
                  <button key={i} onClick={() => handleSizeChange(size)}
                    ...
                    className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-all ${
                      selectedSize?.label === size.label
                        ? "border-[#E771A3] bg-[#FCE4EC] text-[#E771A3]"
                        : "border-gray-300 text-gray-700 hover:border-[#E771A3] hover:text-[#E771A3]"
                    } ${size.stock === 0 ? "opacity-40 cursor-not-allowed line-through" : "cursor-pointer"}`}
          ...
          {/* ✅ BUTTONS — cleaner, fewer, professional */}
          <div className="border-t border-gray-100 pt-5 space-y-3">

            {/* Row 1: Add to Cart + Buy Now */}
            <div className="flex gap-3">
              <button onClick={handleAddToCart} disabled={activeStock === 0}
                aria-label={addedToCart ? "Cart এ যোগ হয়েছে" : "Cart এ যোগ করুন"}
              ...
              {/* ✅ "Buy Now" — WhatsApp e যাবে */}
              <button onClick={handleBuyNow} disabled={activeStock === 0}
                aria-label="WhatsApp এ order করুন"
                className="flex-1 py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 bg-[#E771A3] text-white hover:bg-[#d15f93] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            {/* Row 2: Facebook Message (secondary, outlined) */}
            <button onClick={handleFacebookMessage} disabled={activeStock === 0}
              aria-label="Facebook এ message করুন"
