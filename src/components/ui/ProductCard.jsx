      ...
      */}
            ...
            "
					...
					{product.isBestSeller && product.stock >= 10 && (
						<span className="absolute top-2 left-2 bg-[#FCE4EC] text-[#7D1A44] px-2 py-0.5 text-[10px] md:text-xs rounded-full font-bold">
							⭐ Best Seller
						</span>
					)}
					{hasOffer && (
						<span className="absolute bottom-2 left-2 bg-[#BE3F7A] text-white px-2 py-0.5 text-[10px] md:text-xs rounded-full font-bold">
							{discount}% OFF
						</span>
					)}
				</div>
				...
				{/* ── Info area ─────────────────────────────────────────────── */}
				<div
					className={`flex flex-col flex-1 ${compact ? "p-2 md:p-3" : "px-2 py-2 md:p-4"}`}
				>
					{/* Category + Rating row */}
